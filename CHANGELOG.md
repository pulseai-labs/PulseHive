...
- **`HiveEvent::ToolProgress { agent_id, tool_name, progress }`** — the agent loop forwards each `ToolProgress` from a streaming tool as this event on the `HiveMind::deploy()` stream, so consumers see live progress instead of a frozen wait. Delivery on `deploy()` is **best-effort** (a lossy broadcast that drops events for lagging subscribers); the ordered `Started → … → Completed` envelope is guaranteed only at the emitter/`EventExporter` boundary, so consumers should not treat `Completed` as a guaranteed terminal marker on the `deploy()` stream.
