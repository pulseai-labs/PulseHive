...
- **pulsehive-runtime**: a streaming tool that sends `Started`/`Completed` progress bookends itself no longer duplicates them on the event stream — the progress forwarder drops tool-sent bookends (with a warning), so every tool call keeps exactly the one pair the loop emits.
- **pulsehive-runtime**: a panic inside a tool's `execute`/`execute_streaming` no longer silently kills the agent task — previously the spawned run unwound with no `ToolCallCompleted` and no `AgentCompleted`, leaving the consumer's stream quiet forever. The panic is now caught at the tool-execution boundary and surfaces as an ordinary tool error `tool '<name>' panicked: <message>`: bookends and `ToolCallCompleted` are still emitted and the turn continues as it does for any other tool failure.
- **pulsehive-py / pulsehive-js**: `HiveEvent` variants a binding predates are no longer discarded — the fallback keeps the variant's serde tag as `event_type` (instead of a fieldless `"unknown"`) and carries the full event JSON under `raw` as a JSON string, the same convention as `ToolProgress.progress`.
- **pulsehive-runtime**: a cancelled run no longer enters the record phase — `AgentOutcome::Cancelled` now skips experience extraction entirely, so the default extractor and any custom `ExperienceExtractor` are never invoked after cancellation and cannot start new work (e.g. their own LLM calls) once the run has ended. An extraction already underway for a non-cancelled outcome is still awaited to completion — never aborted — and recording for uncancelled runs is unchanged.
...
- **HiveMind root cancellation (ADR-014).** `HiveMind` owns an internal root `CancellationToken` created at build; every spawned run is linked to it (a child of the task's token when set) without any watcher task outliving its run. `shutdown()` and `Drop` cancel the root — shutdown() cancels running agents, which stop at their next checkpoint or in-flight provider call and end `AgentOutcome::Cancelled`; a tool already running is awaited, never force-aborted. The HiveMind is terminal after `shutdown()`: agents deployed afterwards start cancelled. There is no `abort_handle()`, no `CancellableTool` trait, and `deploy`/`redeploy` signatures are unchanged.
- **`cancellable_tool` example** (`pulsehive-runtime/examples/cancellable_tool.rs`) — stops a live agent turn from the keyboard (Enter) or after `--auto-stop-ms <n>`, prints `outcome: cancelled` with the partial response and the tool calls started, then runs a second task on the same HiveMind to completion (`next turn: complete`). Offline by default on `ScriptedProvider`; live against an OpenAI-compatible endpoint when `OPENAI_API_KEY` is set (optional `OPENAI_BASE_URL`/`OPENAI_MODEL` via `dotenvy`).
...
- **`AgentOutcome::Cancelled { partial_response }` and `AgentOutcome::PartialComplete { responses, errors }`** — the run-level cancellation outcomes (ADR-014). `Cancelled` is the terminal outcome for a run stopped by its task's cancellation token and carries the partial response assembled so far; `PartialComplete` is reserved for a run that finishes with partial results and carries the completed `responses` and the `errors` encountered. Both serialize through the existing tagged `status` field as `"cancelled"` and `"partial_complete"`. The Python and JavaScript bindings map them to `kind`/`outcome` strings `"cancelled"` and `"partial_complete"` — `AgentCompleted` events gain `partial_response`/`partialResponse`, `responses`, and `errors` fields (`responses`/`errors` travel as JSON strings, the same convention as `ToolProgress.progress`) — and a variant the binding predates surfaces as `"unknown"` rather than failing to compile.
- **`Task::with_cancel(token)` and `Task::cancel_token()`** — caller-owned, task-scoped cooperative cancellation (ADR-014). `with_cancel` attaches a `tokio_util::sync::CancellationToken` to a task and `cancel_token()` reads it back; `HiveMind` derives a per-run child token for every agent spawned for the task and threads it through the workflow dispatch, `LoopContext`, and each `ToolContext`; the semantics landed with the agent-loop checkpoints and the HiveMind root token — see the entries above.
- **`ToolContext::cancel`** — every tool invocation receives a child of the run's cancellation token. A tool that chooses to cooperate polls `context.cancel.is_cancelled()` (or awaits `cancelled()`); a tool that ignores it runs to completion exactly as before. There is no `CancellableTool` trait and no `as_cancellable` capability method (ADR-014).
- `tokio-util` is now a dependency of `pulsehive-runtime` (cancellation tokens only).
- **The agent loop now honors the task's cancellation token (ADR-014).** It checks the run token before each LLM call and before each tool call — a cancelled token ends the turn as `AgentOutcome::Cancelled { partial_response }` carrying the latest assistant text, with no `LlmCallStarted` or `ToolCallStarted` emitted for work that never began. Cancelling also aborts the in-flight provider call: each `chat` carries a per-call child token on `LlmConfig.cancel`, merged with the agent definition's own `LlmConfig.cancel` when set so either token aborts the call, and a provider `LlmTransport` error of kind `Cancelled` maps to `AgentOutcome::Cancelled` instead of `AgentOutcome::Error`. A tool already executing is awaited, never force-aborted; a turn whose token never fires is unchanged.
- **`Sequential`, `Parallel` and `Loop` workflow agents now honor the task's cancellation token (ADR-014).** `Sequential` checks the token before dispatching each child and `Loop` at the top of each iteration; a cancelled run — or a child that returns `Cancelled` — ends the composite as `AgentOutcome::Cancelled` carrying the last completed child or iteration response (`Sequential`, `Loop`) or the completed children's responses joined by newline (`Parallel`). `Parallel` always drains its children cooperatively — every spawned child is joined to completion and a child task is never aborted — so a cancellation stops new work at each child's own checkpoints without losing any `AgentCompleted` event.
...
- **The MSRV is now Rust 1.89** (previously 1.83, the PyO3 0.29 floor). `pulsehive-db` 0.7.0 — the dependency this upgrade pins — itself declares `rust-version = "1.89"` (its `sha2` 0.11 transitive dependency requires 1.85), so the workspace's declared minimum was unsatisfiable on a 1.83 toolchain until raised. CI continues to build on latest stable; the onboarding doc's toolchain guidance follows the new floor.
- **`HiveMind::deploy`/`redeploy` now reuse an existing collective instead of always creating a new one.** Both entry points used to unconditionally run `get_or_create_collective("collective-{task.collective_id}")` and replace the task's collective ID with the result, so a task built with `Task::with_collective(.., existing_id)` was silently redirected into a fresh synthetic namespace and never wrote into the collective it named. Deploy now resolves the task's collective first: an ID that already names a collective in the substrate is used as-is (no duplicate collective is created), and only an unknown ID falls back to creating the `collective-{id}` synthetic namespace as before. Consumers that passed an existing collective ID expecting a fresh collective get the existing one now; consumers relying on `Task::new`'s auto-created namespace are unaffected. This is what lets a migrated 0.5.1 collective be deployed onto directly (see the `pulsedb_migration` fixture suite and ADR-012).
- **`HiveMind::deploy` now executes every task in the list, not just the first.** `deploy(agents, tasks)` used to take `tasks[0]` and silently drop the rest. It now runs every agent against every task — the cartesian product `agents × tasks` — resolving each task's collective independently, and keeps a single background Watch task per deployment that fans in every resolved collective (duplicate collectives are subscribed once). Empty-input behavior is unchanged: no agents returns an empty event stream without resolving collectives, and no tasks deploys every agent against a single default empty task. `redeploy` stays single-task by signature.
- **Deployment watch tasks are now tracked per deployment instead of leaking or being aborted by the next deploy.** In 2.0.2 a repeated `deploy` on the same `HiveMind` overwrote the stored watch handle without cancelling it, leaking the earlier watch task and its EventBus sender clone for the hive's lifetime; the first fix attempt aborted the previous handle, which silently stopped `WatchNotification` forwarding for still-running earlier deployments. `deploy` now establishes every watch subscription synchronously before spawning any agent (a fast agent can no longer record into a collective whose subscription does not exist yet — PulseDB Watch does not replay pre-subscription changes), retains each deployment's watch while that deployment's agents are still running, drains a bounded quiet window after the last agent completes so the final records' notifications are still forwarded, ends the watch after that window, prunes finished handles on each subsequent deploy, and `shutdown()`/drop aborts all of them together.
- **`HiveEvent::AgentStarted` and `AgentCompleted` now carry the run's task identity** — `collective_id: CollectiveId` and `task_description: String` — so consumers can attribute a response or error to its task when one agent executes several tasks in a single cartesian deploy; previously the events identified only the generated agent id, name, and kind. External code constructing these variants (struct literals or exhaustive per-variant destructuring) must add the two fields; the `#[non_exhaustive]` `_ => {}` guidance is unchanged. The JS and Python bindings expose them as `collectiveId`/`taskDescription` and `collective_id`/`task_description` event fields.
...
- **BREAKING: the `pulsehive` meta-crate no longer enables the runtime by default.** `pulsehive-runtime` is now optional behind a new `runtime` feature that also enables `pulsehive-core/substrate`; `openai` and `anthropic` are transport-only (neither resolves `pulsehive-runtime` or `pulsehive-db` on its own), and `testing` implies `runtime`. Consumers that deploy agents or use `HiveMind` must add `features = ["runtime"]` to their `pulsehive` dependency; consumers that only construct providers stay on the provider features alone.
- **BREAKING: `pulsehive-core` direct consumers that need storage must now enable the `substrate` feature.** PulseDB is an optional, default-off dependency of core; the `pulsedb` prelude re-exports (`Experience`, `PulseDBSubstrate`, `SubstrateProvider`, …) appear only under `substrate`. The `pulsehive-py` and `pulsehive-js` bindings enable it explicitly.
- **BREAKING: `CollectiveId`, `ExperienceId`, `InsightId` and `RelationId` are now core-owned types** (`pulsehive_core::ids`, re-exported through `pulsehive::prelude`). They no longer alias PulseDB types — there is no PulseDB type identity at the core boundary; `pulsehive-runtime` converts to the wire shape internally (ADR-013). Both families expose public `as_bytes()`/`from_bytes()` for lossless conversion across the boundary — e.g. `pulsedb::CollectiveId::from_bytes(*context.collective_id.as_bytes())` when implementing `ExperienceExtractor`, or `CollectiveId::from_bytes(*db_id.as_bytes())` to target a seeded collective with `Task::with_collective`.
- **BREAKING: `AgentOutcome` is now `#[non_exhaustive]`.** New terminal outcomes — `Cancelled` and `PartialComplete` in this release — can land in a minor release without a major bump. An exhaustive `match` on `AgentOutcome` needs a `_ =>` arm; the Python and JavaScript bindings added one, mapping variants they predate to `"unknown"`.
- **BREAKING: `ToolContext` and `pulsehive_runtime::agentic_loop::LoopContext` gain a `cancel: tokio_util::sync::CancellationToken` field, and `Task` gains a private `cancel` field.** Struct-literal construction of `ToolContext` or `LoopContext` must add the field (use `CancellationToken::new()` for a run no caller can cancel, or a child token to propagate a caller's cancellation), and `Task` struct literals outside `pulsehive-runtime` no longer compile — move to `Task::new(..)` / `Task::with_collective(..)` plus the `Task::with_cancel(..)` builder. Field access on all three types is unchanged.
