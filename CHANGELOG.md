...
- **`Task::with_cancel(token)` and `Task::cancel_token()`** — caller-owned, task-scoped cooperative cancellation (ADR-014). `with_cancel` attaches a `tokio_util::sync::CancellationToken` to a task and `cancel_token()` reads it back; `HiveMind` derives a per-run child token for every agent spawned for the task and threads it through the workflow dispatch, `LoopContext`, and each `ToolContext`. **No behavior changes in this release** — nothing observes the tokens beyond the plumbing itself, so cancelling a token today is visible to code that polls it but aborts nothing; the cancellation semantics land in a follow-up.
