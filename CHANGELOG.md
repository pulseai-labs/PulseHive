...
- **pulsehive-openai**: `finish_reason` and `reasoning` (including the `reasoning_content` alias) come off the wire onto `LlmResponse`; `chat_stream` still carries neither (documented limitation), and the streamed response ends at the `Done` chunk — polling past it yields end-of-stream, so a cancellation after `Done` produces no error.
