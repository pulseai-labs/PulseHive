...
- **`LlmResponse::{finish_reason, reasoning}`** plus the constructors `LlmResponse::new` and `LlmResponse::text` and the builders `with_tool_calls`, `with_usage`, `with_finish_reason`, `with_reasoning`. Providers fill the new fields in a later release; they are `None` until then.
