...
- **BREAKING: `CollectiveId`, `ExperienceId`, `InsightId` and `RelationId` are now core-owned types** (`pulsehive_core::ids`, re-exported through `pulsehive::prelude`). They no longer alias PulseDB types — there is no PulseDB type identity at the core boundary; `pulsehive-runtime` converts to the wire shape internally (ADR-013).
