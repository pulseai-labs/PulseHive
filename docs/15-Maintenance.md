...
| SPEC.md | Repository root | Architecture or design changes | Manual |
...
SPEC.md is versioned (currently v0.4.0-spec). Increment the version with significant changes.
...
- Does it align with the architecture in SPEC.md?
...
- **SPEC.md** captures every architectural decision with rationale.
...
If a new maintainer (human or AI-assisted) takes over, reading CLAUDE.md + SPEC.md provides a complete onboarding path.
...
```
[ ] Review SPEC.md — does it still match reality?
